...
namespace KDTree
{
  ...
  template <typename _Val, typename _Acc, typename _Cmp>
    ...
    {
    public:
      _Node_compare(unsigned int const __DIM, _Acc const& acc, _Cmp const& cmp)
    ...
    private:
      unsigned int _M_DIM;	// don't make this const so that an assignment operator can be auto-generated
      _Acc _M_acc;
      _Cmp _M_cmp;
  };
  ...
  inline
  bool
  _S_node_compare (const unsigned int __dim,
  ...
  _S_node_distance (const unsigned int __dim,
  ...
  inline
  typename _Dist::distance_type
  _S_accumulate_node_distance (const unsigned int __dim,
			       const _Dist& __dist, const _Acc& __acc,
			       const _ValA& __a, const _ValB& __b)
  {
  ...
  inline
  const NodeType*
  _S_node_descend (const unsigned int __dim,
		   const _Cmp& __cmp, const _Acc& __acc,
  ...
  std::pair<const NodeType*,
	    std::pair<unsigned int, typename _Dist::distance_type> >
  _S_node_nearest (const unsigned int __k, unsigned int __dim, SearchVal const& __val,
		   ...
		   _Predicate __p)
  {
    ...
    NodePtr near_node;
    NodePtr far_node;
    unsigned int probe_dim = cur_dim;
    ...
    return std::pair<NodePtr,
      std::pair<unsigned int, typename _Dist::distance_type> >
      (__best, std::pair<unsigned int, typename _Dist::distance_type>
       (__dim, __max));
  }
