...
namespace KDTree
{
  ...
  template <unsigned int const __K, typename _Val, typename _SubVal,
	...
	: _M_acc(__acc), _M_cmp(__cmp)
      {
        for (unsigned int __i = 0; __i != __K; ++__i)
          {
	...
	: _M_acc(__acc), _M_cmp(__cmp)
      ...
      _Region&
      set_high_bound(value_type const& __V, unsigned int const __L)
      {
      ...
      _Region&
      set_low_bound(value_type const& __V, unsigned int const __L)
      {
